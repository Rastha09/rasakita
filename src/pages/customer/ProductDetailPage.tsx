...
export default function ProductDetailPage() {
  ...
  const showDebouncedToast = useCallback(() => {
    ...
    toastTimeoutRef.current = setTimeout(() => {
      toast({ description: 'Keranjang diperbarui', duration: 2000 });
    }, 700);
  }, []);
  ...
  const handleIncrement = () => {
    ...
    if (product.stock > 0 && newQty > product.stock) {
      toast({ description: 'Stok tidak cukup', variant: 'destructive', duration: 2000 });
      return;
    }
    if (isInCart) {
      updateQty(product.id, newQty);
    } else {
      addItem({ product_id: product.id, name: product.name, price: product.price, image: getProductImage() || undefined, qty: 1 });
    }
    showDebouncedToast();
  };
  ...
  const handleDecrement = () => {
    ...
    const newQty = cartQty - 1;
    if (newQty <= 0) { removeItem(product.id); } else { updateQty(product.id, newQty); }
    showDebouncedToast();
  };
  ...
  return (
    <CustomerLayout>
      <div className="pb-28">
        ...
        <ProductReviewSection productId={product.id} ratingAvg={product.rating_avg || 4.7} ratingCount={product.rating_count || 0} />
      </div>
        ...
        <div className="flex items-center gap-4 max-w-lg mx-auto">
          {(isInCart || product.stock > 0) && (
            <div className="flex items-center gap-2 bg-muted rounded-full p-1">
              <Button size="icon" variant="ghost" className="h-10 w-10 rounded-full" onClick={handleDecrement} disabled={cartQty <= 0}>
                <Minus className="h-4 w-4" />
              </Button>
              <span className="w-8 text-center font-semibold">{cartQty}</span>
              <Button size="icon" variant="ghost" className="h-10 w-10 rounded-full" onClick={handleIncrement} disabled={product.stock > 0 && cartQty >= product.stock}>
          ...
          {product.stock <= 0 ? (
            <Button className="flex-1 h-12 rounded-full text-base font-semibold" disabled>Stok Habis</Button>
          ) : isInCart ? (
            <Button className="flex-1 h-12 rounded-full text-base font-semibold" onClick={() => navigate('/cart')}>Lihat Keranjang</Button>
          ) : (
            <Button className="flex-1 h-12 rounded-full text-base font-semibold" onClick={handleAddToCart}>Tambah ke Keranjang</Button>
          )}
        </div>
      </div>
    </CustomerLayout>
